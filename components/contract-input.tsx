...
import Editor from "react-simple-code-editor";
import Prism from "prismjs";
import "prismjs/components/prism-python"; // ✅ Python Prism component
...
// ✅ Validate Python code only
const isValidPython = (code: string) => {
  const pyPattern = /(def\s+\w+\s*\(.*\):|import\s+\w+|print\s*\(.*\))/;
...
const CustomCodeEditor: React.FC<CustomCodeEditorProps> = ({
  ...
  const handleAnalyze = () => {
    if (!isValidPython(contract)) {
      alert("Only Python code is allowed. Please enter valid Python code.");
      return;
    }
    analyze();
  };
